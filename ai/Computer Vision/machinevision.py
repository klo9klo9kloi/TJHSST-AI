...
for colin in range(0, len(matrix)):
  for numan in range(0, len(matrix[0])):
    if colin == 0 or numan == 0 or colin == (len(matrix)-1) or numan == (len(matrix[0])-1):
      ...
      output.write(str(int(avgr)) + ' ')
      output.write(str(int(avgb)) + ' ')
      output.write(str(int(avgg)) + ' ')
...
#find closest center
      ...
      mindist = 9999
      for c in centers:
        dist = int(math.sqrt(math.pow((batman-c[0]),2) + math.pow((superman-c[1]),2)))
        if dist < mindist:
          mindist = dist
          closest = c
'''
